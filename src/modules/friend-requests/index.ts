...
import { FriendRequestHTTPService, FriendNotificationSocketService } from './infras';
...
export const setupFriendRequestHexagon = (
  ...
  const useCase = new FriendRequestUseCase(repository, blockRepository, friendshipRepository, userRepository);
  ...
  router.post('/friend-requests/:receiverId', mdlFactory.auth, httpService.sendFriendRequestAPI.bind(httpService));
  router.post(
    '/friend-requests/:requestId/accept',
    mdlFactory.auth,
    httpService.acceptFriendRequestAPI.bind(httpService)
  );
  router.post(
    '/friend-requests/:requestId/reject',
    mdlFactory.auth,
    httpService.rejectFriendRequestAPI.bind(httpService)
  );
  router.delete('/friend-requests/:requestId', mdlFactory.auth, httpService.cancelFriendRequestAPI.bind(httpService));
  router.get('/friend-requests/received', mdlFactory.auth, httpService.getReceivedRequestsAPI.bind(httpService));
  router.get('/friend-requests/sent', mdlFactory.auth, httpService.getSentRequestsAPI.bind(httpService));
