...
import { GetConversationsCursorQuery, ConversationCursorResult } from "../model/dto";

export class GetConversationsCursorQueryHandler
  implements IQueryHandler<GetConversationsCursorQuery, ConversationCursorResult>
{
  constructor(
  ...
  async query(query: GetConversationsCursorQuery): Promise<ConversationCursorResult> {
    ...
    const pinned = isFirstLoad ? await this.enrichConversations(query.userId, pinnedMembers, true) : null;
    const data = await this.enrichConversations(query.userId, normalMembers, false);
  ...
  private async enrichConversations(
    ...
    conversations.forEach((conv) => {
      if (conv.type === ConversationType.PRIVATE && conv.pairKey) {
        ...
        const targetId = ids.find((id) => id !== userId);
        if (targetId) targetUserIds.add(targetId);
      }
    });

    const targetUsers = targetUserIds.size > 0
    ...
    return conversations.map((conv) => {
      ...
      if (conv.lastMessage && conv.lastMessage.senderId === userId && conv.type === ConversationType.PRIVATE) {
        try {
          const membersInConv = this.conversationMemberQueryRepo.list(
            { conversationId: conv.id },
            { page: 1, limit: 10 },
          ).then((res) => res);
          membersInConv.then((membersInConvArr) => {
            const otherMember = membersInConvArr.find((m) => m.userId !== userId);
            if (otherMember) {
              if (
