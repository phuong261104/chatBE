...
import { sendGroupMessageDTOSchema, SendGroupMessageCommand } from '../model/dto';
...
export class SendGroupMessageHandler implements ICommandHandler<SendGroupMessageCommand, Message> {
  ...
  async execute(command: SendGroupMessageCommand): Promise<Message> {
    ...
    const { success, data: validatedInput, error } = sendGroupMessageDTOSchema.safeParse(command);
    ...
    for (const m of allMembers) {
      if (m.userId !== validatedInput.senderId) {
