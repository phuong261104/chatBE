...
export class MessagingSocketService {
  ...
  private setupEventHandlers() {
    this.namespace.on('connection', (socket: AuthenticatedSocket) => {
      ...
      socket.on('messageSeen', async (payload: MessageSeenPayload, callback) => {
        await this.handleMessageSeen(socket, payload, callback);
      });

      socket.on('messageDelivered', async (payload: MessageDeliveredPayload, callback) => {
  ...
  private async handleMessageSeen(
    ...
    try {
      ...
      if (!conversationId || !lastSeenMessageId) {
        const error = { success: false, error: 'conversationId and lastSeenMessageId are required' };
        if (callback) callback(error);
        return;
      }
      ...
      const memberUserIds = await this.useCase.getConversationMembers(conversationId, userId);
  ...
  private async handleMessageDelivered(
    ...
    try {
      ...
      if (!conversationId || !lastDeliveredMessageId) {
        const error = { success: false, error: 'conversationId and lastDeliveredMessageId are required' };
        if (callback) callback(error);
        return;
      }
  ...
  public notifyMessageSeen(userId: string, conversationId: string, seenByUserId: string, lastSeenMessageId: string) {
