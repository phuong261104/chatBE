...
Luồng chính:
- Gửi tin nhắn bằng HTTP v2 và socket, kiểm tra message được lưu DB, emit `receiveMessage`, TTL tạo `expiresAt/expireAtEpoch` và message hết hạn không còn xuất hiện khi load/search.
- Sửa tin nhắn bằng HTTP v2 và socket, kiểm tra chỉ sửa được trong 30 giây, có `editedAt`, emit `message:edited`, quá hạn bị từ chối.
- Delivered/read dùng HTTP v1 và socket vì chưa có v2, kiểm tra `lastDeliveredMessageId`, `lastSeenMessageId`, `lastReadMessageId`, `unreadCount`.
